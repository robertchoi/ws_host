...
sql_sp_score_table = "select * from sp_score_table"
...
def parsing_data(data):
    ...
    if startorfinish == '1': # '1' 
        ...
        if dicLabTime[bandId].total_seconds() < str2sec('00:05:00'):
            ...
            if startorfinish == '2':
                dicTagScore[bandId] = dicTagScore[bandId] + tag1_score
            elif startorfinish == '3':
                dicTagScore[bandId] = dicTagScore[bandId] + tag2_score
...
def fetch_score():
    cursor.execute(sql_sp_score_table) 
    result = cursor.fetchall()
    df = pd.DataFrame(result)
    if df.empty:
        print('fetch_score DataFrame is empty!')
        return
    print(df)

    global tag1_score, tag2_score, tag3_score, tag4_score
    tag1_score = df.iloc[0,1]
    tag2_score = df.iloc[0,2]
    tag3_score = df.iloc[0,3]  
    tag4_score = df.iloc[0,4]   
    print("tag1_score", tag1_score)
